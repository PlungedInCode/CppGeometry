...
bool Circle::operator==(const Shape& another) const {
    ...
    if (otherCircle) {
        // Compare circle parameters for equality
        return center() == otherCircle->center() && radius_ == otherCircle->radius_;
    }
...
bool Circle::isCongruentTo(const Shape& another) const {
    ...
    if (otherCircle) {
        // Two circles are congruent if they have the same center and radius_
        return center() == otherCircle->center() && radius_ == otherCircle->radius_;
    }
...
void Circle::scale(const Point& center, double coefficient) {
    // Scale the radius_ of the circle and move its center accordingly
    radius_ *= coefficient;
    // center().scale(center, coefficient);
}